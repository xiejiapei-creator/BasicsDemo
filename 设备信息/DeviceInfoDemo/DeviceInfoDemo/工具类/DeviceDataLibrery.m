...
@property (nonatomic, assign) DiviceType device;// 设备类型
...
+(instancetype)sharedLibrery
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        // 转化为设备型号
        _librery.device = [self transformMachineToIdevice];
    });
    return _librery;
}
...
{
    return DeviceNameContainer[self.device];
}

// 获取设备初始系统型号
- (const NSString *)getInitialVersion
{
    return initialFirmwareContainer[self.device];
}

// 获取设备支持的最高系统型号
- (const NSString *)getLatestVersion
{
    return latestFirmwareContainer[self.device];
}
...
{
    return BatteryCapacityContainer[self.device];
}

// 获取电池电压，单位 V 福特
- (CGFloat)getBatterVolocity
{
    return BatteryVoltageContainer[self.device];
}
...
// 设备名称
static const NSString *DeviceNameContainer[] =
{
    [iPhone_1G]                 = @"iPhone 1G",
    [iPhone_3G]                 = @"iPhone 3G",
